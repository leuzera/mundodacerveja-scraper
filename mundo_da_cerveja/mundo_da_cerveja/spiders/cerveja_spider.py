...
class LinksRotulosSpider(scrapy.Spider):
    ...
    start_urls = [
            ...
            'https://www.mundodacerveja.com/pesquisa/9/8',
            ...
            'https://www.mundodacerveja.com/pesquisa/33/8',
    ...
    def parse(self, response):
        for href in response.css('.boxItens a::attr(href)'):
            yield response.follow(href, self.parse_cerveja)

    def parse_cerveja(self, response):
        base_url = get_base_url(response)
        for cerveja in response.css('.col-right'):
            name = cerveja.css('.dados h2::text').extract_first(),
            image_urls = cerveja.css('img ::attr(src)').extract()
